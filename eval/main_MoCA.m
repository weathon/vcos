...
salDir = '/home/wg25r/flowsam/no_mom_full/'; %MoCA
% salDir = '/home/wg25r/flowsam/no_neg/no_negative_prompt/'

values = [0.03, 0.05, 0.07, 0.09, 0.11, 0.13];
names = {};
for i = 1:length(values)
    names{end+1} = sprintf('%.2f', values(i));
end

% for i = 1:length(values)
%     names{end+1} = sprintf('no_tracking_%.2f', values(i));
% end

% names{end+1} = 'moca-mask-te'

disp(names);
Models = names;

gtDir = '/home/wg25r/fastdata/interval/MoCA_Video/'; 
Datasets = {'TestDataset_per_sq'}; %ValDataset_per_sq TestDataset_per_sq
Results_Save_Path = './';


Thresholds = [0.5]%1:-1/255:0;

totalFileID = fopen("all_in_one.csv", "a")
fprintf(totalFileID, "(model),seq_wFm,seq_Sm,seq_meanEm,seq_MAE,seq_meanDic,seq_meanIoU,seq_maxEm,seq_maxDice,seq_maxIoU,seq_meanSen,seq_maxSen,seq_meanSpe,seq_maxSpe\n")
...
for  m = 1:length(Models)
    ...
    for videonum = 1:length(Datasets)
        ...
        for seqnum = 1: seqNUM
            ...
            % seqfolder = convertStringsToChars(seqFiles(seqnum+2))%.name;
        ...
        fprintf('(Dataset) wFm  Sm  meanEm  MAE meanDic meanIoU maxEm maxDice maxIoU meanSen maxSen meanSpe maxSpe\n (%s) %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n',...
            videofolder,video_wFm(videonum),video_Sm(videonum),meanEm, video_mae(videonum),meanDic,meanIoU,maxEm,maxDic,maxIoU,meanSen,maxSen,meanSpe,maxSpe);
        fprintf(totalFileID, '(%s),%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n',...
                modelName,video_wFm(videonum),video_Sm(videonum),meanEm,video_mae(videonum),meanDic,meanIoU,maxEm,maxDic,maxIoU,meanSen,maxSen,meanSpe,maxSpe);
    end
