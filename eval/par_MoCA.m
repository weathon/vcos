...
salDir = '/home/wg25r/flowsam/'; %MoCA
Models = {'Pred'};
gtDir = '/home/wg25r/fastdata/real/MoCA_Video/'; 
...
for m = 1:length(Models)
    for videonum = 1:length(Datasets)
        ...
        videoNUM = length(videoFiles) + 1;
        [tmp_video_Sm, tmp_video_wFm, tmp_video_mae] = deal(zeros(1,videoNUM));
        [tmp_video_column_E, tmp_video_column_Sen, tmp_video_column_Spe, tmp_video_column_Dic, tmp_video_column_IoU] = deal(zeros(videoNUM,256));
        ...
        %MAE
        tmp_video_mae(videonum) = mean2(seq_mae);
        %Sm
        tmp_video_Sm(videonum) = mean2(seq_Sm);
        %wFm
        tmp_video_wFm(videonum) = mean2(seq_wFm);
        %E-m
        tmp_video_column_E(videonum,:) = mean(seq_column_E, 1);
        meanEm = mean(tmp_video_column_E(videonum,:));
        maxEm = max(tmp_video_column_E(videonum,:));
        %Sensitivity
        tmp_video_column_Sen(videonum,:) = mean(seq_column_Sen, 1);
        meanSen = mean(tmp_video_column_Sen(videonum,:));
        maxSen = max(tmp_video_column_Sen(videonum,:));
        %Specificity
        tmp_video_column_Spe(videonum,:) = mean(seq_column_Spe, 1);
        meanSpe = mean(tmp_video_column_Spe(videonum,:));
        maxSpe = max(tmp_video_column_Spe(videonum,:));
        %Dice
        tmp_video_column_Dic(videonum,:) = mean(seq_column_Dic,1);
        meanDic = mean(tmp_video_column_Dic(videonum,:));
        maxDic = max(tmp_video_column_Dic(videonum,:));
        %IoU
        tmp_video_column_IoU(videonum,:) = mean(seq_column_IoU,1);
        meanIoU = mean(tmp_video_column_IoU(videonum,:));
        maxIoU = max(tmp_video_column_IoU(videonum,:));
        
        fprintf(fileID, '(Dataset) wFm Sm meanEm MAE meanDic meanIoU maxEm maxDice maxIoU meanSen maxSen meanSpe maxSpe\n (%s) %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n',...
                videofolder,tmp_video_wFm(videonum),tmp_video_Sm(videonum),meanEm,tmp_video_mae(videonum),meanDic,meanIoU,maxEm,maxDic,maxIoU,meanSen,maxSen,meanSpe,maxSpe);
