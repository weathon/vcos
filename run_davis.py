...
for thresh in [0.25]:
    ...
    for i, video in enumerate(tqdm.tqdm(available)):
        ...
        ret = os.system(f"python3 davis.py --video_name {video} --log_path log3.csv --output_dir davis16_{thresh} --threshold {thresh} --use_motion_detection --use_bgs --positive_prompt 'salient center object NAME highlighted in blue, solid object only' --box_only")  
        if ret != 0:
